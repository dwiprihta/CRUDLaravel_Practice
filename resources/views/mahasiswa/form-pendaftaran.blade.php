...
<div class="container pt-5">
    <div claass="row">
        ...
        <div class="card" style="width: 100%;">
            <div class="card-body">
                 <h5 class="card-title">PENDAFTARAN MAHASISWA</h5><hr>
                 <form action="{{ route('mahasiswas.store') }}" method="POST">
                    @csrf
                    <div class="form-group">
                        <label for="nim">Nomor Induk Mahasiswa (NIM)</label>
                        <input type="text" class="form-control @error('nim') is-invalid @enderror" name="nim" id="nim" placeholder="Nim..." value="{{old ('nim')}}">
                        @error('nim')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('nama') is-invalid @enderror" name="nama" id="nim" placeholder="Nama..." value="{{old ('nama')}}">
                        @error('nama')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <option value="Teknik Perkebunan"{{old('jurusan')=='Teknik Perkebunan' ? 'selected': '' }}>Teknik Perkebunan</option>
                            </select>
                            @error('jurusan')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
